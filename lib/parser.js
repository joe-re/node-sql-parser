...
var FromClauseParser = require('../base/fromClauseParser')
...
exports.parseFromClause = function (sql) {
  const matched = sql.match(/\s?(FROM\s(?:.|\s)*)/);
  const target = matched ? matched[1] : sql;
  return FromClauseParser.parse(target)
}
